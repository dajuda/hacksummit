define( [], function(){

	var questions = [
		{ value: 'Today I went to the <strong>doctor</strong>',             correct: 'doctor',      url: 'data/img/signs/doctor.gif'	},
		{ value: 'Leonardo DiCaprio is a great <strong>actor</strong>.',    correct: 'actor',       url: 'data/img/signs/actor.gif'		},
		{ value: "I'm proud to be an <strong>American</strong>.",           correct: 'american',    url: 'data/img/signs/american.gif'	},
		{ value: '<strong>Bats</strong> are scary.',                        correct: 'bat',         url: 'data/img/signs/bat.gif'		},
		{ value: '<strong>Cats</strong> are adorable.',                     correct: 'cat',         url: 'data/img/signs/cat.gif'		},
		{ value: 'Chocolate <strong>ice cream</strong> is my favorite.',    correct: 'icecream',    url: 'data/img/signs/icecream.gif'	},
		{ value: 'The book is <strong>on</strong> the table',               correct: 'on',          url: 'data/img/signs/on.gif'		},
		{ value: 'Call the <strong>police</strong>!',                       correct: 'police',      url: 'data/img/signs/police.gif'	},
		{ value: 'Obama is the current US <strong>president.</strong>',     correct: 'president',   url: 'data/img/signs/president.gif'	},
		{ value: 'Which is the <strong>program</strong> today?',            correct: 'program',     url: 'data/img/signs/program.gif'	},
		{ value: 'Have you got some <strong>progress</strong>?',            correct: 'progress',    url: 'data/img/signs/progress.gif'	},
		{ value: 'Is the <strong>shower</strong> working?',                 correct: 'shower',      url: 'data/img/signs/shower.gif'	},
		{ value: 'This <strong>singer</strong> has a great voice.',         correct: 'singer',      url: 'data/img/signs/singer.gif'	},
		{ value: 'Please <strong>sit</strong> down.',                       correct: 'sit',         url: 'data/img/signs/sit.gif'		},
		{ value: '<strong>Sorry</strong> about that.',                      correct: 'sorry',       url: 'data/img/signs/sorry.gif'		},
		{ value: 'Do you <strong>speak</strong> well in public?',           correct: 'speak',       url: 'data/img/signs/speak.gif'		},
		{ value: '<strong>Stay</strong> still',                             correct: 'stay',        url: 'data/img/signs/stay.gif'		},
		{ value: "How many <strong>steps</strong> have you walked today?",  correct: 'step',  		url: 'data/img/signs/step.gif'		},
		{ value: 'Which wood is this <strong>table</strong> made of?',      correct: 'table',       url: 'data/img/signs/table.gif'		},
		{ value: 'Please <strong>take</strong> this',                       correct: 'take',        url: 'data/img/signs/take.gif'		},
		{ value: '<strong>Wake up</strong> you are late',                   correct: 'wake-up',     url: 'data/img/signs/wake-up.gif'	},
		{ value: 'Jequitibá is a beautiful <strong>tree</strong>.',         correct: 'tree',        url: 'data/img/signs/tree.gif'		},
	];

	return questions;
})
